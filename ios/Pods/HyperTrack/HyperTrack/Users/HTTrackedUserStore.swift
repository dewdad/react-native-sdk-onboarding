...
internal protocol TrackedUserStoreDelegate : class {
    ...
    func didUpdateActionsFor(collectionId: String, actions: [HyperTrackAction]?)
...
final class HTTrackedUserStore {
    ...
    weak var delegate: TrackedUserStoreDelegate?
    var isTimerOn = false
    ...
    @objc func onAppForeground(_ notification: Notification) {
        if isTimerOn{
            initializeTimer()
        }
    }
    ...
    func trackActionFor(collectionId: String, completionHandler: ((_ actions: [HyperTrackAction]?,
        _ error: HyperTrackError?) -> Void)?) {
        
        self.fetchDetailsFor(collectionId: collectionId, completionHandler: { (users, error) in
            // Process trackAction response
            self.trackedUserDataSource.collectionId = collectionId
            self.processTrackActionResponse(collectionId: collectionId, users, error, completionHandler: completionHandler)
        })
    }
    ...
    private func fetchDetailsFor(collectionId: String,
                                 completionHandler: @escaping (_ users: [HTTrackedUser]?,
        _ error: HyperTrackError?) -> Void) {
        RequestManager().fetchDetailsForActionsByCollectionId(collectionId, completionHandler: { (users, error) in
            if let error = error {
                completionHandler(nil, error)
                return
            }
    ...
    private func processTrackActionResponse(collectionId: String?, _ users: [HTTrackedUser]?,
                                            _ error: HyperTrackError?,
                                            completionHandler: ((_ actions: [HyperTrackAction]?,
        _ error: HyperTrackError?) -> Void)?) {
        if error != nil {
        ...
        if let users = users {
            ...
            // Start timer to poll for updates when a look up id is tracked
            if trackedUserDataSource.collectionId != nil {
                self.initializeTimer()
            } else if trackedUserDataSource.getActionIdsList().count > 0 {
                for actionId in trackedUserDataSource.getActionIdsList() {
                    if let action = trackedUserDataSource.getAction(actionId: actionId) {
    ...
    func getActions(users: [HTTrackedUser]?) -> [HyperTrackAction] {
        var userActions = [HyperTrackAction]()
        if let users = users {
            if users.count > 0 {
                for user in users {
    ...
    @objc private func updateLocations() {
        if let collectionId = trackedUserDataSource.collectionId {
            fetchDetailsFor(collectionId: collectionId, completionHandler: { (users, _) in
                let oldActions = self.trackedUserDataSource.trackedActionsMap
                self.clearAction()
                self.trackedUserDataSource.collectionId = collectionId
                self.trackedUserDataSource.addTrackedUsers(users: users)
                let newActions = self.trackedUserDataSource.trackedActionsMap
                self.delegate?.didUpdateActionsFor(collectionId: collectionId, actions: self.getActions(users: users))
        ...
        } else if let lookUpId = trackedUserDataSource.lookupId {
            fetchDetailsFor(lookUpId: lookUpId, completionHandler: { (users, _) in
                let oldActions = self.trackedUserDataSource.trackedActionsMap
                self.clearAction()
                self.trackedUserDataSource.lookupId = lookUpId
                self.trackedUserDataSource.addTrackedUsers(users: users)
            ...
            fetchDetailsFor(actionIds: trackedUserDataSource.getUnCompleteActionIdsList(),
                            completionHandler: { (users, error) in
                                if let error = error {
                                    DDLogError("error in fetching detail list \(error.description)")
                                    return
                                }

...
@objc public class HTSDKActivity: HTSegment {
    ...
    public func toHyperTrackActivity() -> HyperTrackActivity {
        let activity = HyperTrackActivity.init(id: self.lookupId, type: self.type.rawValue, activity: self.unknownReason?.rawValue, startedAt: self.startTime, endedAt: self.endTime, place: nil, distance: nil, stepCount: nil, stepDistance: nil, encodedPolyline: nil, timeAwarePolyline: nil, distanceDisplayUnit: nil, reason: unknownReason?.rawValue)
        return activity
    }
}

...
class HTSDKControlsService: NSObject {
    ...
    var currentSDKControls: HyperTrackSDKControls? = nil
    var currentServerSDKControls: HyperTrackSDKControls? = nil
    ...
    func setUpControls(){
        if let currentControls = self.getCurrentSavedSDKControls() {
            ...
            currentSDKControls = HyperTrackSDKControls.getDefaultControls()
        }
    ...
    func getCurrentSavedSDKControls() -> HyperTrackSDKControls?{
        if let data  = HypertrackUserDefaults.standard.data(forKey: currentControlKey){
            if let sdkControls = HyperTrackSDKControls.fromJson(data: data){
                return sdkControls
            }
        }
        return nil
    ...
    func onSDKControlsChange(newSDKControls: HyperTrackSDKControls){
    ...
    func onNewSDKControls(newControls: HyperTrackSDKControls, force: Bool = false){
        
        if force == true{
            if let serverControls = self.currentServerSDKControls {
                if serverControls.recordedAt.timeIntervalSince1970 + (serverControls.ttl ?? HyperTrackSDKControls.minimumTTL) > Date().timeIntervalSince1970 {
    ...
    func saveControls(controls: HyperTrackSDKControls) {
        let dict = controls.toDict()
        do {
            let jsonData = try JSONSerialization.data(withJSONObject: dict, options: .prettyPrinted)
            HypertrackUserDefaults.standard.set(jsonData, forKey: currentControlKey)
    ...
    func refreshTTLTimerOfSDKControls(controls: HyperTrackSDKControls) {
    ...
    func reEvaluateSDKControls(){
        ...
        if canGoAggressive{
            ...
            onNewSDKControls(newControls: HyperTrackSDKControls.getDefaultControls(), force: true)
        }
    }
    
    @objc func resetSDKControls() {
        DDLogInfo("Resetting sdk controls")
        HypertrackUserDefaults.standard.removeObject(forKey: currentControlKey)
        HypertrackUserDefaults.standard.synchronize()
    }
    ...
    func getAggressiveSDKControls() -> HyperTrackSDKControls{
        let controls =  HyperTrackSDKControls.init(userId: HTUserService.sharedInstance.userId, runCommand: HTSDKControlsRunCommand.goActive.rawValue , ttl: maxTTL, minimumDuration: 5.0, minimumDisplacement: 5.0, batchDuration: 5.0)
        return controls
    }
}

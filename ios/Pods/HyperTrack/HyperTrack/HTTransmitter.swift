...
final class Transmitter {
  ...
  func createUser(_ name:String, completionHandler: ((_ user: HyperTrackUser?, _ error: HyperTrackError?) -> Void)?) {
    self.requestManager.createUser(["name":name]) { user, error in
      ...
      if (user != nil) {
        ...
        debugPrint("Error creating user: ", error as Any)
      }
  ...
  func assignAction(_ expectedPlace: [String: Any], _ type: String, _ completionHandler: @escaping (_ action: HyperTrackAction?, _ error: HyperTrackError?) -> Void) {
    var action = [

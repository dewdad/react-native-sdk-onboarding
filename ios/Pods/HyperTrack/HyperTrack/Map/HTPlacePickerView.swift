...
@objc public class HTPlacePickerView: UIView, MKMapViewDelegate, UISearchBarDelegate {
    ...
    func setUpView() {
        ...
        } else if let location = HTSDKDataManager.sharedInstance.locationManager.getLastKnownLocation() {
            region =    MKCoordinateRegionMake(
                (location.coordinate),
                MKCoordinateSpanMake(0.005, 0.005))
        } else {
...
extension HTPlacePickerView: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if  let cell = tableView.dequeueReusableCell(withIdentifier: "SearchCell", for: indexPath) as?HTSearchViewCell {
...
extension HTPlacePickerView: MapCustomizationDelegate {
    ...
    func mapMarkerForDestination(annotation: HTMapAnnotation) -> MKAnnotationView {
        ...
        if let markerView: HTMarkerWithTitle = bundle.loadNibNamed("MarkerTitleView", owner: self, options: nil)?.first as? HTMarkerWithTitle {

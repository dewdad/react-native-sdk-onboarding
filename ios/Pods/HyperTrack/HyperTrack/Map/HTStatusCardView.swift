...
class HTStatusCardView: UIView {
    ...
    var statusDelegate: StatusViewDelegate?
    ...
    func shrink() {
        if !self.isShrinked {
            ...
            let y = self.frame.origin.y + (self.expandedCard?.frame.size.height)!/2.0
            let width = self.frame.size.width
            let height = self.frame.height - (self.expandedCard?.frame.size.height)!/2.0
    ...
    func expand() {
        if self.isShrinked {
            ...
            let x = self.frame.origin.x
            let y = self.frame.origin.y - (self.expandedCard?.frame.size.height)!
            let width = self.frame.size.width
            let height = (self.expandedCard?.frame.size.height)! + self.frame.height
            self.frame = CGRect(x: x, y: y, width: width, height: height )
    ...
    func addExpandedCard() {
        ...
        if let expandedCard: ExpandedCard = bundle.loadNibNamed("ExpandedCard", owner: self, options: nil)?.first as? ExpandedCard {
    ...
    func addCompletedView () {
        let bundle = Settings.getBundle()!
        if let completedView: CompletedView = bundle.loadNibNamed("CompletedView", owner: self, options: nil)?.first as? CompletedView {

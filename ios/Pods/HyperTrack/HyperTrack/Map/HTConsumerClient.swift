...
internal protocol HTConsumerClientDelegate: class {
...
class HTConsumerClient: NSObject, TrackedUserStoreDelegate {
    ...
    weak var delegate: HTConsumerClientDelegate?
    ...
    func getCollectionId() -> String? {
        return trackedUserStore?.trackedUserDataSource.collectionId
    }
    ...
    func trackActionFor(collectionId: String, delegate: HTConsumerClientDelegate,
                        completionHandler: ((_ actions: [HyperTrackAction]?, _ error: HyperTrackError?) -> Void)?) {
        self.delegate = delegate
        trackedUserStore?.trackActionFor(collectionId: collectionId, completionHandler: { (actions, error) in
            if error != nil {
                if let completionHandler = completionHandler {
                    completionHandler(nil, error)
                }
    ...
    func didUpdateActionsFor(collectionId: String, actions: [HyperTrackAction]?) {
        self.eventDelegate?.didRefreshData?(forCollectionId: collectionId, actions: actions)
    }

...
@objc public class PlacelineView: UIView {
    ...
    func initPlacelineView(date: Date?, userId: String? = nil) {
        ...
        self.userId = userId
        if userId == nil {
            self.userId = HTUserService.sharedInstance.userId
        }
    ...
    func getPlaceLineData() {
        if HyperTrack.getUserId() != nil {
            HyperTrack.getPlaceline(date: date, userId: userId) { (placeLine, _) in
...
extension PlacelineView: UITableViewDataSource, UITableViewDelegate {
    ...
    public  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "placeCell", for: indexPath) as! PlaceLineCell
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        guard let cell = placeLineTable.cellForRow(at: indexPath) as? PlaceLineCell else { return }
        ...
        if !self.noResults {
            ...
            if selectedIndexPath != nil && selectedIndexPath?.row != indexPath.row {
                if let oldCell = self.placeLineTable.cellForRow(at: selectedIndexPath!) as? PlaceLineCell {
                    oldCell.normalize()
                }
            }
    ...
    public func tableView(_ tableView: UITableView, didDeselectRowAt indexPath: IndexPath) {
        let cell = tableView.cellForRow(at: indexPath) as? PlaceLineCell
        cell?.deselect()
    }
}

...
enum LiveLocationViewState: Int {
    case search = 0
    case locationConfirm = 1
    case locationSelected = 2
    case trackingStarted = 3
    case trackingCompleted = 4
}
...
class HTLiveLocationView: HTCommonView {
    ...
    var destinationAnnotation: HTMapAnnotation?
    var currentUserAnnotation: HTMapAnnotation?
    ...
    var currentViewState = LiveLocationViewState.search {
        didSet {
    ...
    var tripSummaryCard: TripSummaryView?
    var tripSummarySuperView: UIView?
    var expandedCard: ExpandedCard?
    var completedView: CompletedView?
    var haveShownInitialMarkers = false
    var lastUserLocation: CLLocationCoordinate2D?
    var savedPlaces: [HyperTrackPlace]?
    var locationSelectionType = LocationSelectionType.searchResult
    ...
    func initMapView(mapProvider:MapProviderProtocol,interactionViewDelegate: HTViewInteractionInternalDelegate) {
        ...
        if HTConsumerClient.sharedInstance.getCollectionId() != nil {
            currentViewState = LiveLocationViewState.trackingStarted
        }
        ...
        if(currentViewState == LiveLocationViewState.trackingStarted) {
    ...
    func setUpSearchResultTableView() {
        ...
        if(currentViewState != LiveLocationViewState.search) {
            ...
            if let userAnnotation = currentUserAnnotation {
                if let location = HTSDKDataManager.sharedInstance.locationManager.getLastKnownLocation() {
                    ...
                    if (lastUserLocation != nil) {
                        UIView.animate(withDuration: 0.2, animations: {userAnnotation.coordinate = location.coordinate}, completion: { (_) in
    ...
    func onStopTrip(notification: Notification) {
        currentViewState = LiveLocationViewState.search
    ...
    func onStateChange(currentState: LiveLocationViewState, fromState: LiveLocationViewState) {
        ...
        } else if (currentViewState == LiveLocationViewState.search) {
            self.refocusButton.isHidden = true
        ...
        } else if (currentViewState == LiveLocationViewState.trackingCompleted) {
            self.refocusButton.isHidden = true
        }
        ...
        if(currentViewState == LiveLocationViewState.locationConfirm) {
    ...
    func didSelectedLocation(place: HyperTrackPlace, selectionType: LocationSelectionType) {
        ...
        currentViewState = LiveLocationViewState.locationSelected
        self.resetStatusView()
        ...
        if(!(selectionType == LocationSelectionType.myLocation || selectionType ==  LocationSelectionType.onMap)) {
    ...
    func onStartTracking(action: HyperTrackAction, userIds: [String]) {
        ...
        if (currentViewState == LiveLocationViewState.locationSelected) {
            currentViewState = LiveLocationViewState.trackingStarted
        }
    ...
    func canShareTracking(userIds: [String]) -> Bool {
        if(userIds.count == 1) {
            if(HTUserService.sharedInstance.isCurrentUser(userId: userIds.first)) {
                return true
            }
        }
        return false
    ...
    func canStopTracking(userIds: [String]) -> Bool {
        for userId in userIds {
            if(HTUserService.sharedInstance.isCurrentUser(userId: userId)) {
                return true
            }
        }
        return false
    ...
    func isTripCompleted(userIds: [String]) -> Bool {
        ...
        for userId in userIds {
            ...
            if let  action = user?.actions?.last {
                let statusInfo = HTStatusCardInfo.getUserInfo(action!, userId, useCase: HTConstants.UseCases.singleUserSingleAction, isCurrentUser: HTUserService.sharedInstance.isCurrentUser(userId: userId))
    ...
    func showTripSummary(action: HyperTrackAction, userIds: [String]) {
        ...
        if(userIds.count == 1) {
            ...
            let statusInfo = HTStatusCardInfo.getUserInfo(action, action.user?.id, useCase: HTConstants.UseCases.singleUserSingleAction, isCurrentUser: HTUserService.sharedInstance.isCurrentUser(userId: action.user?.id))
    ...
    func updateStatusCard(action: HyperTrackAction, userIds: [String]) {
        ...
        if(isTripCompleted(userIds: userIds)) {
            currentViewState = LiveLocationViewState.trackingCompleted
            showTripSummary(action: action, userIds: userIds)
        } else {
            if(userIds.count == 1) {
                if(self.statusCard == nil) {
    ...
    func animateMarker(userId: String,
        ...
        if let heroAnnotation = (self.mapViewDataSource?.getMapViewModel(userId: userId)?.heroMarker) {
            ...
            if let coordinates = locations as [CLLocationCoordinate2D]?, coordinates.count >= 1 {
                ...
                UIView.animate(withDuration: duration, animations: {heroAnnotation.coordinate = currentLocation}, completion: { (_) in
    ...
    func isCurrentUserSharingLocation() -> Bool {
        let userIds = HTConsumerClient.sharedInstance.getUserIds()
        let currentUserId = HTUserService.sharedInstance.userId
        if(userIds.count > 0) {
...
extension HTLiveLocationView: UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
            ...
            if let location = HTSDKDataManager.sharedInstance.locationManager.getLastKnownLocation() {
                getSearchResultsForCoordinate(cordinate: location.coordinate, completionHandler: { place, error in
                    if(error == nil && place != nil) {
                        if indexPath.row == 0 {
                            self.locationSelectionType = LocationSelectionType.myLocation
                            self.didSelectedLocation(place: place!, selectionType: LocationSelectionType.myLocation)
                        } else {
                            self.locationSelectionType = LocationSelectionType.onMap
                            self.didSelectedLocation(place: place!, selectionType: LocationSelectionType.onMap)
                        }
                    }
                })
            }
        ...
        if(isShowingSavedResults) {
            ...
            locationSelectionType = LocationSelectionType.savedResult
            self.didSelectedLocation(place: location, selectionType: LocationSelectionType.savedResult)

        } else {
            location = (searchResults?[indexPath.row])!
            locationSelectionType = LocationSelectionType.searchResult
            self.didSelectedLocation(place: location, selectionType: LocationSelectionType.searchResult)
...
extension HTLiveLocationView: MapCustomizationDelegate {
    ...
    func mapView(_ mapView: MKMapView, regionWillChangeAnimated animated: Bool) {
        ...
        if(shouldStartTrackingRegion && currentViewState != LiveLocationViewState.trackingCompleted && (currentViewState == LiveLocationViewState.locationSelected || currentViewState == LiveLocationViewState.locationConfirm)) {
            if(currentViewState != LiveLocationViewState.locationConfirm) {
                currentViewState = LiveLocationViewState.locationConfirm
                self.interactionViewDelegate?.willChooseLocationOnMap?()
            }
        ...
        } else if (currentViewState == LiveLocationViewState.trackingStarted && haveShownInitialMarkers ) {
    ...
    func mapView(_ mapView: MKMapView, regionDidChangeAnimated animated: Bool) {
        if(shouldStartTrackingRegion && (currentViewState == LiveLocationViewState.locationSelected || currentViewState == LiveLocationViewState.locationConfirm)  && currentViewState != LiveLocationViewState.trackingStarted) {
    ...
    func mapMarkerForHero(annotation: HTMapAnnotation) -> MKAnnotationView {
        ...
        if let title = annotation.title {
            ...
            if let action = annotation.action {
                if(HTUserService.sharedInstance.isCurrentUser(userId: action.user?.id)) {
        ...
        if let user = annotation.action?.user {
            if(HTUserService.sharedInstance.isCurrentUser(userId: user.id) ) {
            }
        }
...
extension HTLiveLocationView: UITextFieldDelegate {
    public func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        if(currentViewState == LiveLocationViewState.locationConfirm  || currentViewState == LiveLocationViewState.locationSelected) {
            currentViewState = LiveLocationViewState.search
            resetViewToStartSearch()
        }
        return true

...
@objc public final class HTMap: NSObject {
        ...
        if (userIds.count > 0) {
            if (userIds.count == 1) {
                if (HTConsumerClient.sharedInstance.getActions(userId:userIds.first!)?.count == 1) {
                    self.useCase = HTConstants.UseCases.singleUserSingleAction
                    
                } else {
                    self.useCase = HTConstants.UseCases.singleUserMultipleAction
                }
            } else {
                self.useCase =  HTConstants.UseCases.multipleUserMultipleAction
            }
        }
    }
    ...
    func onUpdateAction(userId:String, actionId:String){
        ...
        if (self.useCase == HTConstants.UseCases.singleUserSingleAction) {
    ...
    func resetAddressInfo(_ actionIdToBeUpdated: String?){
        ...
        if self.isAddressViewShown == true {
            ...
            if (self.useCase == HTConstants.UseCases.singleUserSingleAction), (action.display != nil) {

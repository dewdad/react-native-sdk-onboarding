...
class HTCommonView: UIView {
    ...
    var useCase = HTConstants.UseCases.singleUserSingleAction
    ...
    func setUpHeroMarker(userId: String, coordinates: [CLLocationCoordinate2D],disableHeroMarkerRotation:Bool, polylineCordinates: [CLLocationCoordinate2D]) {
        ...
        if let  action = user?.actions?.last as? HyperTrackAction {
            ...
            if (self.useCase == HTConstants.UseCases.singleUserSingleAction), (action.display != nil), (action.display?.showSummary == true) {
                return
            }
...
//    func animatePolyline(locations: [CLLocationCoordinate2D],polylineCordinates:[CLLocationCoordinate2D],duration: TimeInterval,userId: String, currentIndex: Int){
//        
...
//                        
//                        print(currentIndex)
////                        self.animatePolyline(locations: locations, polylineCordinates: polylineCordinates, duration: duration, userId: <#T##String#>, currentIndex: <#T##Int#>)
//                        
//                        

...
class HTSDKDataManager: NSObject, HTActivityEventsDelegate, HTHealthEventsDelegate, LocationEventsDelegate {
    ...
    func startMockTracking(origin: CLLocationCoordinate2D?, destination: CLLocationCoordinate2D?, completionHandler: ((_ error: HyperTrackError?) -> Void)?) {
    ...
    func locationManager(_ manager: LocationManagerProtocol, didUpdateLocations locations: [HyperTrackLocation]) {
    ...
    func didChangeActivityTo(activity: HTSDKActivity, fromActivity: HTSDKActivity?) {
        ...
        if let htLocation = self.locationManager.getLastKnownHyperTrackLocation(){
    ...
    func saveActionCompletedEvent(lookUpId: String){
        guard let userId = HTUserService.sharedInstance.userId else { return }
        
        let event = HyperTrackEvent.init(userId: userId, sessionId: Settings.sessionId, deviceId: Settings.deviceId, recordedAt: Date(), eventType: HyperTrackEventType.actionCompleted, activityLookUpId: "", locationLookUpId: "", healthLookUpId: "", data: ["lookup_id": lookUpId])
    ...
    func saveTrackingStarted() {
        ...
        let event = HyperTrackEvent.init(userId: userId, sessionId: Settings.sessionId, deviceId: Settings.deviceId, recordedAt: Date(), eventType: HyperTrackEventType.trackingStarted , activityLookUpId: "", locationLookUpId: "", healthLookUpId: "", data: [:])
    ...
    func saveMockTrackingStarted(){
        ...
        let event = HyperTrackEvent.init(userId: userId, sessionId: Settings.sessionId, deviceId: Settings.deviceId, recordedAt: Date(), eventType: HyperTrackEventType.trackingStarted , activityLookUpId: "", locationLookUpId: "", healthLookUpId: "", data: ["is_mock":true])
    ...
    func saveTrackingEnded() {
        ...
        let event = HyperTrackEvent.init(userId: userId, sessionId: Settings.sessionId, deviceId: Settings.deviceId, recordedAt: Date(), eventType: HyperTrackEventType.trackingEnded , activityLookUpId: "", locationLookUpId: "", healthLookUpId: "", data: [:])

...
@objc public class HyperTrackPlace: NSObject {
    ...
    public static func placeFromDict(dict: [String: Any]?) -> HyperTrackPlace? {
        guard let dict = dict else {
            return nil
        }
        
        var id = dict["id"] ?? ""
        var geolocation = dict["location"]
        var htLocation : HTGeoJSONLocation? = nil
        if let location = geolocation {
            htLocation = HTGeoJSONLocation.fromDict(dict: location as! [String : Any])
        }
        
        let locality = dict["locality"] ?? ""
        let landmark = dict["landmark"] ?? ""
        let zipCode = dict["zip_code"] ?? ""
        let city = dict["city"] ?? ""
